...
pub fn encipher(data: &[u8], keys: &[u32; 4]) -> Vec<u8> {
    ...
    let mut index = 0;
    for _ in 0..blocks {
        let mut v0 = u32::from_be_bytes([data[index], data[index + 1], data[index + 2], data[index + 3]]);
        let mut v1 = u32::from_be_bytes([data[index + 4], data[index + 5], data[index + 6], data[index + 7]]);
        let mut sum = 0_u32;
        for _ in 0..ROUNDS {
